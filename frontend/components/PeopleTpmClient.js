...
export default function PeopleTpmClient({ user, initialAgencies = [] }) {
  ...
  const editorTitle = editor?.kind === "person" ? `${editor.person ? "Edit" : "Add"} Person` : editor?.kind === "reassign" ? "Reassign Sub-Agent Number" : `${editor?.code ? "Edit" : "Add"} Sub-Agent Number`;
  ...
  return (
    <div className="people-page">
      <header className="people-page-header">
        <div><p className="people-eyebrow">People directory</p><h1 ref={pageHeading} tabIndex={-1}>People &amp; Sub-Agent Numbers</h1><p>Each person can have multiple Sub-Agent Numbers. Manage their details, codes and assignments here.</p></div>
        <div className="people-actions">
          ...
          {canCreate ? <button className="people-button primary" disabled={actionsBlocked || !accessiblePeople.some((person) => canForAgency(user, person.agency, "can_create"))} onClick={(event) => openEditor("code", null, null, event)}><Plus size={18} aria-hidden="true" />Add Sub-Agent Number</button> : null}
        </div>
      </header>

      <section className="people-filters" aria-label="Search and filters">
        <div className="people-field people-search"><label htmlFor="people-search">Search people or Sub-Agent Numbers</label><div><Search size={18} aria-hidden="true" /><input id="people-search" type="search" placeholder="Name or Sub-Agent Number" value={filters.query} onChange={(event) => updateFilter("query", event.target.value)} /></div></div>
        ...
        <label className="people-checkbox"><input type="checkbox" checked={filters.showInactive} onChange={(event) => updateFilter("showInactive", event.target.checked)} />Show inactive Sub-Agent Numbers</label>
      ...
      {editor ? <section className="people-editor" aria-labelledby="people-editor-title">
        <div className="people-section-heading"><div><p className="people-eyebrow">{editor.kind === "person" ? "Person details" : "Sub-Agent Number details"}</p><h2 id="people-editor-title" ref={editorHeading} tabIndex={-1}>{editorTitle}</h2></div><button className="people-button secondary" type="button" disabled={pending} onClick={closeEditor}><X size={16} aria-hidden="true" />Cancel</button></div>
        {editor.code ? <dl className="people-assignment-summary"><div><dt>Sub-Agent Number</dt><dd>{editor.code.code}</dd></div><div><dt>Current person</dt><dd>{editor.person.full_name}</dd></div><div><dt>Current agency</dt><dd>{editor.person.agency_name}</dd></div><div><dt>Current status</dt><dd><Badge active={editor.code.is_active} /></dd></div></dl> : null}
        <form onSubmit={save} aria-busy={pending}>
          <fieldset disabled={pending} className="people-form-fields">
            ...
            {editor.kind === "person" ? <>
              ...
              {editor.kind !== "reassign" ? <Field name="code" label="Sub-Agent Number" errors={errors}>{(attrs) => <input {...attrs} required maxLength={80} value={draft.code} onChange={(event) => updateDraft("code", event.target.value)} />}</Field> : <div className="people-field"><span>New agency</span><p className="people-readonly">{target?.agency_name || "Select a new person to see their agency."}</p></div>}
            </>}
            {editor.kind === "person" || !editor.code || editor.kind === "reassign" ? <Field name="is_active" label={editor.kind === "person" ? "Person status" : "Sub-Agent status after saving"} errors={errors}>{(attrs) => <select {...attrs} value={draft.is_active ? "true" : "false"} onChange={(event) => updateDraft("is_active", event.target.value === "true")}><option value="true">Active</option><option value="false">Inactive</option></select>}</Field> : null}
          </fieldset>
      ...
      <section aria-labelledby="people-results-title" aria-busy={state.loading}>
        <div className="people-section-heading people-results-heading"><div><h2 id="people-results-title">People directory</h2><p role="status">{state.loading ? "Loading people…" : `${visiblePeople.length} matching people · ${codeCount} matching Sub-Agent Numbers`}</p></div><button className="people-button secondary" disabled={blocked} onClick={loadPeople}><RotateCcw size={16} aria-hidden="true" />{state.loading ? "Refreshing…" : "Refresh"}</button></div>
        {!state.loading && !visiblePeople.length ? <div className="people-empty"><Users size={32} aria-hidden="true" /><h3>No people match your filters.</h3><p>Try a different name, Sub-Agent Number or agency, or reset your filters.</p><button className="people-button secondary" onClick={resetFilters}>Reset filters</button></div> : null}
        <div className="people-cards">
          {visiblePeople.map((person) => <article className="people-card" key={person.id} aria-labelledby={`person-${person.id}`}>
            <header className="people-card-header"><div className="people-identity"><h3 id={`person-${person.id}`}>{person.full_name}</h3><p><Building2 size={15} aria-hidden="true" />{person.agency_name}</p><div className="people-person-meta"><Badge active={person.is_active} /><span>{person.agent_type === "SUBAGENT" ? "Sub-agent" : "Main Agent"}</span><span>{person.tpm_codes?.length || 0} Sub-Agent Numbers</span></div></div><div className="people-actions">{canForAgency(user, person.agency, "can_edit") ? <button className="people-button secondary" disabled={actionsBlocked} onClick={(event) => openEditor("person", person, null, event)}><Edit3 size={16} aria-hidden="true" />Edit person</button> : null}{canForAgency(user, person.agency, "can_create") ? <button className="people-button primary" disabled={actionsBlocked} onClick={(event) => openEditor("code", person, null, event)}><Plus size={16} aria-hidden="true" />Add Sub-Agent Number</button> : null}</div></header>
            <ul className="people-code-list" aria-label={`Sub-Agent Numbers for ${person.full_name}`}>
              {person.visibleCodes.map((code) => <li className="people-code-row" key={code.id}><div className="people-code-identity"><span className="people-code-label">Sub-Agent Number</span><strong>{code.code}</strong><span>Terminal: {code.terminal_number || "Not assigned"}</span><Badge active={code.is_active} /></div><div className="people-actions" role="group" aria-label={`Actions for Sub-Agent Number ${code.code}`}>{codeActions(user, person, code).map((action) => { const [Icon, label, style] = actionDetails[action]; return <button key={action} className={`people-button ${style}`} disabled={actionsBlocked} aria-label={`${label} Sub-Agent Number ${code.code}`} onClick={(event) => action === "edit" || action === "reassign" ? openEditor(action === "edit" ? "code" : "reassign", person, code, event) : setCodeStatus(person, code)}><Icon size={16} aria-hidden="true" />{label}</button>; })}</div></li>)}
            </ul>
            {!person.visibleCodes.length ? <p className="people-no-codes">{person.tpm_codes?.length ? "No Sub-Agent Numbers match the current filters. Show inactive codes to see more." : "No Sub-Agent Numbers assigned yet."}</p> : null}
          </article>)}
        </div>
      </section>
    </div>
